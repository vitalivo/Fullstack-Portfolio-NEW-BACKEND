import smtplib
import requests
import ssl # <-- Добавляем импорт ssl
from email.mime.text import MIMEText
from email.mime.multipart import MIMEMultipart
from django.conf import settings
# from django.template.loader import render_to_string # Не используется в твоем коде
# from django.utils.html import strip_tags # Не используется в твоем коде
import logging
...
def send_email_notification(contact_message):
    ...
    try:
        ...
        # Текстовая версия
        text_content = f"""🔔 НОВОЕ СООБЩЕНИЕ С ПОРТФОЛИО
👤 ОТПРАВИТЕЛЬ:
...
Админка: http://127.0.0.1:8000/admin/contacts/contactmessage/{contact_message.id}/change/
        ...
        # НЕ ИСПОЛЬЗУЙТЕ В ПРОДАКШЕНЕ! Это делает ваше соединение уязвимым.
        ...
        with smtplib.SMTP('smtp.gmail.com', 587) as server:
            server.starttls(context=context) # <-- Передаем наш контекст сюда
